  ...
  const loadRequests = async (page = 1) => {
    try {
      ...
      if (checkUserRole('المدير العام')) {
        endpoint = `/requests?request_type=4&page=${page}`;
      } else if (
        ...
        endpoint = `/requests?request_type=4&branch=${branch}&page=${page}`;
      } else {
      ...
      setCurrentPage(dummyData.current_page);
      setLastPage(dummyData.last_page);
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-[70vh] p-4">
            ...
            <table className="w-full table-fixed">
              <tbody>
                ...
                {filteredData.length === 0 && (
                  <tr>
                    <td colSpan={7} className="text-center py-6 text-gray-500">لا توجد طلبات</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <div className="flex gap-4 mt-6">
        <button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} className="px-4 py-2 bg-gray-300 rounded disabled:opacity-50">السابق</button>
        <span className="self-center">صفحة {currentPage} من {lastPage}</span>
        <button disabled={currentPage === lastPage} onClick={() => setCurrentPage(currentPage + 1)} className="px-4 py-2 bg-gray-300 rounded disabled:opacity-50">التالي</button>
      </div>
