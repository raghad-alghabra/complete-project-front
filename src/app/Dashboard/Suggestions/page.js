  ...
  const loadRequests = async (page = 1) => {
    try {
      ...
      if (checkUserRole('المدير العام')) {
        endpoint = `/requests?request_type=5&page=${page}`;
      } else if (
        ...
        endpoint = `/requests?request_type=5&branch=${branch}&page=${page}`;
      } else {
        endpoint = `/requests?request_type=0&page=${page}`;
      }

      const dummyData = await fetchData(endpoint);
      console.log('البيانات المستلمة:', dummyData);
      ...
      <div className="flex justify-center items-center w-full overflow-x-auto">
        <div className="w-full max-w-6xl overflow-x-auto">
          ...
          <div className="flex justify-center mt-4 gap-2">
            ...
            <span className="px-3 py-1">صفحة {currentPage} من {lastPage}</span>
            <button
